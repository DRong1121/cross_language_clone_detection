...
sys.path.append('.')
sys.path.append('../../../')
import core.transcoder.dataloaders.preprocessing.dictionary as dic

import core.transcoder.dataloaders.preprocessing.lang_processors as langp
import core.transcoder.dataloaders.preprocessing.lang_processors.utils as lputils
import core.transcoder.dataloaders.preprocessing.lang_processors.obfuscation.utils_deobfuscation as deobf

from core.transcoder.dataloaders.preprocessing.bpe_modes import FastBPEMode, RobertaBPEMode
from core.transcoder.dataloaders.utils import extract_dict, restore_roberta_segmentation_sentence
...
class CodeTokenizer(Transform[str, TokCode]):
    ...
    def __getstate__(self) -> tp.Dict[str, tp.Any]:
        return extract_dict(self, reset_keys=["_tokenizer"])
...
class BpeBase(Transform[TokCode, str]):
    ...
    # (because fast_bpe is not picklable)
    def __getstate__(self) -> tp.Dict[str, tp.Any]:
        return extract_dict(self, reset_keys=["_bpe_model"])
